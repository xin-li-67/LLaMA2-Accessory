...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if self.training:  # for training
            ...
            # )
            scale = 1 / query_states.shape[-1] ** 0.5
            query_states = query_states * scale
            attn = query_states @ key_states.transpose(-2, -1)
            attn = attn.softmax(-1)
...
class Transformer(PreTrainedModel):
    ...
    def __init__(self, config: BaiChuanConfig, with_visual=False):
...
class BaiChuan(PreTrainedModel):
    def __init__(self, config: BaiChuanConfig):
        super().__init__(myconfig)
        self.model = Transformer(myconfig)
